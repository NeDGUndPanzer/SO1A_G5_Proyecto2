      ...
      if (data && data.length) {
        ...
        for (let i = 0; i < data.length; i++) {
          if (typeof data[i] != 'undefined') {
            let arr=[];
            let t=1;
            data[i].ultimos_vacunados.map(a=>(
                arr.push({id: t++, nombre: a.nombre})
            ))
            data2[p] = { key: j++, pais: data[i]._id, ultimos_vacunados: arr};
            p++;
          }
        }
      } else {
                    ...
                    <div id={`tabla-${index1}`} key={`${index1}`}>
                        ...
                        <div className={classes.container} style={{ height: 400, width: '100%', backgroundColor: '#FFF' }}>
                            <MuiThemeProvider theme={theme}>
                                <DataGrid rows={Element.ultimos_vacunados} columns={columns2} pageSize={5} />
                            </MuiThemeProvider>
                        </div>
                        <br></br>
                    </div>
