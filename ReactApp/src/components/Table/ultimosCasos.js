...
export default function UltimosCasosRegistrados() {
  let [data2, setData] = useState([]);

  const getData = async () => {
    try {
      //const { data } = await axios.get('http://localhost:5000/ultimosCasosRegistrados');
      const { data } = await axios.get('http://34.72.201.120:4000/ultimosCasosRegistrados');
      if (data && data.length) {
