...
export default function RangoEdades() {
    ...
    const getData = async () => {
        try {
            // const { data } = await axios.get('http://localhost:5000/rangoEdades');
            const { data } = await axios.get('https://us-central1-nifty-inkwell-308322.cloudfunctions.net/rangoEdades');
            ...
            if (data && data.length) {
                ...
                for (let i = 0; i < data.length; i++) {
                    if (typeof data[i] != 'undefined') {
                        data2[p] = { key: j++, pais: data[i].pais, rangos: data[i].rangos};
                        let ran = [];
                        data2[p].rangos.forEach(element => {
                            ran.push({label: element.name, y: element.total})
                        });
                        graficarPuntos[p] =
                        {
                            exportEnabled: true,
